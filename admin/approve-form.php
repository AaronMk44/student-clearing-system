...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if ($admin->adminRole == 'accountant') {
    $form->accountantApprovalStatus = 'approved';
    $form->accountantApprovalDate = date('Y-m-d', time());
    $form->accountsRunningBalance = floatval(InputFilter::sanitizeField($_POST['runningBalance']));
    $fm->update($form);

    header('location: pending-forms.php');
  }

  // -------------------------------------------------


  if ($admin->adminRole == 'librarian') {
    $form->librarianApprovalStatus = 'approved';
    $form->librarianApprovalDate = date('Y-m-d', time());
    $form->libraryItemsDue = InputFilter::sanitizeField($_POST['itemsDue']);
    $fm->update($form);
  ...
  if ($admin->adminRole == 'hostel_representative') {
    $form->hostelRepApprovalStatus = 'approved';
    $form->hostelRepApprovalDate = date('Y-m-d', time());
    $form->hostelItemsDue = InputFilter::sanitizeField($_POST['itemsDue']);
    $fm->update($form);
  ...
  if ($admin->adminRole == 'hod') {
    $form->hodApprovalStatus = 'approved';
    $form->hodApprovalDate = date('Y-m-d', time());
    $fm->update($form);
  ...
  $_SESSION['flashmessage'] =
    '<div class="alert alert-success alert-dismissible" role="alert">
     Room Clearance Request successfully approved.
          ...
          <div class="container flex-grow-1 container-p-y">
            ...
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb breadcrumb-style1">
                ...
                <li class="breadcrumb-item">
                  <a href="#">Approve Clearance Request</a>
                </li>
              </ol>
            </nav>
              ...
              <div class="d-flex align-items-end row">
                <div class="col-sm-7">
                  <div class="card-body">
                    ...
                    <p class="mb-4">
                      Approve a room <span class="fw-bold">clearance</span> request before the term closes.
                    </p>
                  </div>
                </div>
            ...
            <div class="card">
              <div class="card-header">
                <h4>Clearance Request Form Details</h4>
              </div>
              <div class="card-body">
